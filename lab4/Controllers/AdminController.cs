...
public class AdminController : Controller
{
    ...
    #region Switch to owner

    [HttpPost("SwitchToOwner")]
    public async Task<IActionResult> SwitchToOwnerAsync([FromForm] string password)
    {
        try
        {
            var ownerUser = await userManager.Users.FirstOrDefaultAsync(u => u.UserName == "Owner");
            if (ownerUser is null)
                throw new("There is no owner user");

            if (ownerUser.PasswordHash != password)
                throw new("Password is not valid");

            await signInManager.SignInAsync(ownerUser, true);
